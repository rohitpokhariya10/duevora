...
const { default: VoucherType } = await import("../../../shared/models/voucherType.model.js");
...
describe("Voucher Types Management Integration Tests", () => {

    describe("POST /api/voucher-types", () => {
        it("should successfully create a voucher type", async () => {
            ...
            expect(res.body.data.name).toBe("Journal Voucher");
            expect(res.body.data.code).toBe("JV01"); // formatted uppercase
            expect(res.body.data.description).toBe("Manual journal entries");
            expect(res.body.data.organizationId.toString()).toBe(orgId.toString());
        });

        it("should return conflict if code already exists in organization", async () => {
        ...
        it("should allow duplicate code across different organizations", async () => {
            await VoucherType.create({
                name: "Journal Voucher",
                code: "JV01",
                organizationId: orgId
        ...
        it("should return forbidden if user does not have VOUCHERTYPES.CREATE permission", async () => {
            const res = await request(app)
                .post("/api/voucher-types")
