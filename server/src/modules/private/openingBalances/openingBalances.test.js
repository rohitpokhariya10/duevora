...
const { default: OpeningBalance } = await import("../../../shared/models/openingBalance.model.js");
...
beforeEach(async () => {
    ...
    await Permission.create({ name: "Create Opening Balances", code: "OPENINGBALANCES.CREATE", module: "openingBalances" });
...
describe("Opening Balances Management Integration Tests", () => {
    let fy, account;
    ...
    beforeEach(async () => {
        ...
        account = await Account.create({ name: "Cash", code: "CASH", type: "asset", organizationId: orgId });
    });

    describe("POST /api/opening-balances", () => {
        it("should successfully create an opening balance", async () => {
            const res = await request(app)
                .post("/api/opening-balances")
                .set("Authorization", `Bearer ${adminUserToken}`)
                .send({ financialYearId: fy._id, accountId: account._id, debit: 10000, credit: 0 });

            expect(res.status).toBe(201);
            expect(res.body.success).toBe(true);
            expect(res.body.data.debit).toBe(10000);
        });

        it("should return conflict if opening balance already exists for same fy+account", async () => {
            await OpeningBalance.create({ organizationId: orgId, financialYearId: fy._id, accountId: account._id, debit: 5000, credit: 0 });
            const res = await request(app)
                .post("/api/opening-balances")
                .set("Authorization", `Bearer ${adminUserToken}`)
                .send({ financialYearId: fy._id, accountId: account._id, debit: 1000, credit: 0 });
        ...
        it("should return forbidden without permission", async () => {
            const res = await request(app)
                ...
                .send({ financialYearId: fy._id, accountId: account._id, debit: 500 });
